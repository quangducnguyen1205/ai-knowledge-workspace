...
import com.aiknowledgeworkspace.workspacecore.search.application.port.in.AssetIndexingUseCase;
...
class AssetIndexingKafkaListener {
    ...
    private final AssetIndexingUseCase assetIndexingUseCase;

    AssetIndexingKafkaListener(AssetIndexingUseCase assetIndexingUseCase) {
        this.assetIndexingUseCase = assetIndexingUseCase;
    }
    ...
    void onMessage(ConsumerRecord<String, String> record, Acknowledgment acknowledgment) {
        try {
            AssetIndexingHandleResult result = assetIndexingUseCase.handle(record.value());
