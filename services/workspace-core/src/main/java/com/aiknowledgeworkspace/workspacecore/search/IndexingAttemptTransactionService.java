...
@Service
class IndexingAttemptTransactionService {
    ...
    IndexingAttemptTransactionService(
    ...
    IndexingAttempt beginAttempt(UUID indexingJobId) {
        return transactionTemplate.execute(status -> beginIndexingAttempt(indexingJobId));
    }

    AssetSearchIndexExecutionResult finalizeSuccessfulAttempt(UUID indexingJobId) {
        return transactionTemplate.execute(status -> finalizeAttempt(indexingJobId));
    }

    void markJobFailed(UUID indexingJobId, RuntimeException exception) {
    ...
    void persistBestEffortDiagnostic(UUID indexingJobId, String diagnostic) {
        try {
            transactionTemplate.executeWithoutResult(status -> searchIndexJobRepository.findById(indexingJobId)
                    .ifPresent(indexingJob -> {
