...
P3-C2B `[ĐÃ SMOKE THỰC TẾ]` verifies this backend boundary with real Keycloak runtime: the local profile and realm/client import worked, Authorization Code + PKCE issued signed access tokens, direct grant was not used, Spring validated issuer/signature/audience, and provider plus `sub` mapped to Spring-owned local users. First JWT use created one local default workspace, repeated use resolved the same user, another JWT subject could not access the first user's workspace, and legacy login/session-only identity was rejected in `keycloak_jwt` mode. Frontend bearer-token integration, browser login UX, token refresh/logout UX, default auth cutover, legacy-session removal, and collaboration/membership/RBAC remain future work.
