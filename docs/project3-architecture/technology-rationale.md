...
Current implementation note: Phase 3D-F keeps the Phase 3C Kafka publisher foundation and Spring-side handling foundations for FastAPI result events. Kafka publishing is selected only when `WORKSPACE_CORE_KAFKA_ENABLED=true`; the relay remains disabled by default and has no scheduler. The publisher sends a JSON event envelope to `asset.processing.requested.v1` with event metadata plus the existing payload JSON. Spring can now manually parse and validate `asset.processing.result.v1` envelopes for `transcript.ready` v1 and `asset.processing.failed` v1, persist consumed-event idempotency by `eventId`, and apply product state only after validation. It does not add an automatic Kafka listener, FastAPI repository changes, a scheduler, dead-letter routing, stuck-`PUBLISHING` recovery, Kafka transactions, Schema Registry, Avro, or Protobuf.
