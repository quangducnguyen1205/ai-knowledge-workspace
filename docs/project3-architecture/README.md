...
Historical implementation note: the early Phase 3I/D2/D4 experiments used an explicit opt-in
Kafka transition. The current Spring backend has one request path: durable request intent plus
Kafka processing/result and automatic indexing under the `project3` profile. The historical
direct Spring processing path was removed before timestamp-aware transcript work.

Spring validates `transcript.ready` v1 and `asset.processing.failed` v1 envelopes and records consumed-event idempotency in PostgreSQL by `eventId`. Result events correlate to the original Spring `asset.processing.requested` event ID: `payload.processingRequestId` must equal `causationEventId`, and Spring matches the job by asset ID plus `ProcessingJob.processingRequestEventId`. For `transcript.ready`, Spring retrieves transcript artifact rows by `processingRequestId`, validates the complete row set, and only then replaces its product-owned snapshot and marks the asset ready. Exact-ID operator recovery covers one selected durable consumed-result failure or stale `PUBLISHING` request row. Bounded automatic reconciliation covers only terminal publication rows with typed transient failures; historical, unknown, permanent and recovery-exhausted rows remain manual. Delivery remains at-least-once, so consumers remain idempotent.
...
Spring automatic processing request relay, FastAPI consumer/Celery/MinIO
