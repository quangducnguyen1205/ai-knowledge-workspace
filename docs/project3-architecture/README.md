...
Current implementation note: Phase 3C has introduced local single-node Kafka infrastructure, the `asset.processing.requested.v1` topic bootstrap, and a Spring Kafka publisher adapter behind the outbox relay boundary. The PostgreSQL outbox row remains durable publication intent and Kafka is only event transport. The relay has no scheduler and remains disabled by default; publishing happens only when Kafka is explicitly enabled and the relay is invoked. FastAPI event consumption, Celery handoff, dead-letter routing, Kafka transactions, and stuck-`PUBLISHING` recovery are not implemented yet. Delivery is at-least-once, so future consumers must be idempotent. The existing direct FastAPI upload call remains as the temporary processing trigger until the later async processing lifecycle replaces direct byte streaming with object-key events.
