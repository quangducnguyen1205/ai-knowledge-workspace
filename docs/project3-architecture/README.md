...
Current implementation note: Phase 3D-F keeps the Phase 3C Kafka publisher foundation, adds the Spring-side result-handler foundation for `asset.processing.result.v1`, and makes the upload processing trigger explicit. `WORKSPACE_CORE_PROCESSING_TRIGGER_MODE=direct_upload` is the default product behavior: Spring still calls FastAPI direct upload, stores the direct FastAPI task/video IDs, does not create a Kafka request outbox row, and leaves `ProcessingJob.processingRequestEventId` null. `WORKSPACE_CORE_PROCESSING_TRIGGER_MODE=kafka_request` is an explicit local/manual transition mode: Spring does not call FastAPI direct upload, persists `Asset`, `ProcessingJob`, and one `asset.processing.requested` outbox event atomically, and stores that outbox event ID on `ProcessingJob.processingRequestEventId`. Request relays remain manual/disabled by default, and the two request paths are mutually exclusive per upload to prevent duplicate processing before cutover.

Spring validates `transcript.ready` v1 and `asset.processing.failed` v1 envelopes, records consumed-event idempotency in PostgreSQL by `eventId`, and can manually apply product-state transitions. Result events correlate to the original Spring `asset.processing.requested` event ID: `payload.processingRequestId` must equal `causationEventId`, and Spring matches the job by asset ID plus `ProcessingJob.processingRequestEventId`. `ProcessingJob.fastapiTaskId` remains the transitional direct-upload/FastAPI task identifier and is not used for Kafka result correlation. For `transcript.ready`, Spring retrieves transcript artifact rows from FastAPI by `processingRequestId`, validates the complete row set, and only then replaces its product-owned transcript snapshot and marks the asset ready. No automatic Kafka listener, retry topic, DLQ, FastAPI repository change, Kafka transactions, Schema Registry, Avro, or Protobuf are implemented yet. Delivery remains at-least-once, so Spring dedupes result events by `eventId` and future consumers must remain idempotent.
