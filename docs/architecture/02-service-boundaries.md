...
- Explicit processing trigger modes. The normal `project3` profile selects `kafka_request`,
  which persists the request outbox row and does not call FastAPI direct upload. The
  `direct_upload` mode remains a deprecated compatibility path in the `compatibility` profile
  and does not create a Kafka request outbox row.
