...
The Project3 v1 baseline separates product logic from internal media/AI processing. Spring
Boot is the product core. FastAPI is an internal processing/provider service. Elasticsearch is
...
- One processing request path. Upload persists the request outbox row; the coherent `project3`
  profile enables Kafka and the scoped automatic request relay.
- A disabled-by-default base configuration for the automatic request relay. When enabled with
  Kafka, it selects due `asset.processing.requested` rows only, reuses the durable outbox state
  machine, and does not relay result, indexing, or arbitrary outbox events.
...
Phase 3I keeps Kafka as transport, not product truth. `consumed_processing_result_events` stores durable idempotency by `eventId`; product state changes only after Spring validates the result and persists a complete transcript snapshot. The integrated `project3` profile enables request relay/publisher, result listener, and automatic indexing request/relay/listener. Bounded reconciliation covers typed transient publication failures, but there is no retry topic, Kafka DLQ, broad stale-row repair, or automatic replay of ambiguous historical failures.

P3-D1 added the scoped automatic request relay foundation. It is not a generic outbox worker: it selects a bounded batch of due `asset.processing.requested` events only and runs only with Kafka enabled. Kafka publication remains at-least-once; claim/finalize/failure database updates stay separate from the Kafka send so the scheduler does not hold a product database transaction across the broker call.

P3-D2 `[ĐÃ SMOKE THỰC TẾ]` verifies the historical async processing path with Kafka and FastAPI/Celery: a Spring upload created the product asset/job/request outbox row, the automatic request relay published exactly one `asset.processing.requested` event, FastAPI processed the MinIO object through Celery/Whisper, the result relay published one `transcript.ready` event, and Spring marked the inbox `APPLIED`, job `SUCCEEDED`, and asset `TRANSCRIPT_READY` with a canonical snapshot. A duplicate result ID was an already-applied no-op.

P3-D4 `[ĐÃ SMOKE THỰC TẾ]` verifies the historical fully automatic variant after a one-time local Docker bootstrap: Spring automatic request relay, FastAPI consumer/Celery, FastAPI automatic result relay and Spring automatic result listener completed one MinIO-backed request without manual relay, result-file handling, recovery command or fabricated Kafka injection.
...
P3-E2 `[ĐÃ SMOKE THỰC TẾ]` verifies the complete automatic chain from one upload through processing and derived indexing. Spring published the durable request, FastAPI processed the MinIO object and published the terminal result, Spring persisted the canonical snapshot and indexing intent, and the indexing listener wrote Elasticsearch documents and marked the asset `SEARCHABLE`. PostgreSQL product-state gating hid a stale derived document after the asset state changed.
